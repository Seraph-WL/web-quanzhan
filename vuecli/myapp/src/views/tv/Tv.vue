...
export default {
  ...
  methods: {
    ...
    getData() {
      if(this.isGoOn){
        this.isGoOn = false;
        // https://m.douban.com/rexxar/api/v2/subject_collection/tv_domestic/items?os=android&for_mobile=1&callback=jsonp1&start=0&count=18&loc_id=108288&_=0
        // 将&callback=jsonp1删掉 count=18每次刷新18条，start=0从第0个开始
        let requestUrl =
          `https://m.douban.com/rexxar/api/v2/subject_collection/tv_domestic/items?os=android&for_mobile=1&start=${this.start}&count=10`;
        // 真正访问的地址，代理请求，把豆瓣地址当做参数传递
        ...
        axios.get(baseUrl+requestUrl).then(res => {
          ...
          if(this.dataList.length < res.data.total){//仅在有可获取数据时请求借口如果超过可获取数据长度就不让下次请求生效
            this.isGoOn = true;
          }
        });
      }
...
<style scoped>
  ...
  li .left{
    flex-grow: 1;
    width: 0;
    /* width: 0：保证flex-grow按照比例划分，保证他的宽度不会影响分配的空间 */
  }
  li .left img{
    width: 100%;
  }
